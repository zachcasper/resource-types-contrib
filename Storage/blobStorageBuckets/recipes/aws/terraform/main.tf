    ...
    Statement = [
      {
        ...
        Principal = {
          ...
          AWS = "arn:aws:iam::${var.context.aws.account}:root"
        }
...
output "result" {
  value = {
    ...
    resources = [
      "/planes/aws/aws/accounts/${var.context.aws.account}/regions/${aws_s3_bucket.blob_storage.region}/providers/AWS.S3/Bucket/${aws_s3_bucket.blob_storage.id}",
      "/planes/aws/aws/accounts/${var.context.aws.account}/regions/global/providers/AWS.IAM/Role/${aws_iam_role.s3_access.name}"
    ]
  }
